...
class Engine(object):
    ...
    def schedule(self, scheduling_request):
        ...
        schedule_end = schedule_start.add_days(
            config.scheduling_days.value - 1)  # inclusive
        daily_info_end = schedule_start.add_days(
            config.daily_info_days.value - 1)  # inclusive
        ...
        self.policy.set_default_urgency(config.default_urgency.value)
        ...
        usable_time_dict = self.usable_time_parser.get_usable_time_dict(
        ...
        overdue_tasks = self.task_filter.get_overdue_tasks(
            schedulable_tasks, schedule_start)
        response.alerts.overdue_tasks = overdue_tasks

        schedulable_tasks = self.task_repeater.repeat(
            schedulable_tasks, schedule_start, schedule_end)
        stress_contributor_tasks_mask = self.task_filter.get_stress_contributor_tasks_mask(
            schedulable_tasks, schedule_start, schedule_end)
        self.task_filter.assign_implicit_stressless(
            schedulable_tasks, stress_contributor_tasks_mask)
        ...
        strong_progress = self.progress_counter.count(
            schedulable_tasks, strong_dated_sessions, sessions_sorted=True)
        ...
        bad_estimate_tasks = self.task_filter.get_bad_estimate_tasks(
            schedulable_tasks, strong_progress)
        ...
        early_schedule = Schedule.from_usable_time_dict(
        ...
        for daily_info in response.daily_infos:
            daily_info.actual_usable_time.value = early_schedule.get_usable_time(
                daily_info.date)
        ...
        highest_stress_task = self.stress_analyzer.get_highest_stress_task(
            schedulable_tasks, highest_workload_date)
        ...
        schedule_with_suggested.add_dated_sessions(
            strong_fragment_sessions)
        planner_result = self.planner.plan(
            schedulable_tasks,
        ...
        strong_progress_with_fragments = self.progress_counter.count(
            schedulable_tasks, fragment_sessions, sessions_sorted=True)
        ...
        for daily_info in response.daily_infos:
            sessions = early_schedule.get_sessions(
                daily_info.date, SessionOrder.AMOUNT)
            self.stress_analyzer.compute_session_extra_info(
                daily_info.date, sessions, schedulable_tasks)
            daily_info.sessions = sessions

        sessions = schedule_with_optimal.get_sessions(
            schedule_start, SessionOrder.AMOUNT)
        self.stress_analyzer.compute_session_extra_info(
            schedule_start, sessions, schedulable_tasks)
        response.today_optimal_sessions = sessions
